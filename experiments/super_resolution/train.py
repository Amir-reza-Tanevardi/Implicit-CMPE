...
def superres_degrade(image, scale=2):
    """
    Downsamples a 28x28 Fashion-MNIST image to 14x14.

    Parameters:
    ----------
    image : 2D numpy array (28x28)
    scale : int
        Downsampling factor (default: 2 → 14x14)
    """

    H, W = image.shape
    lr = resize(image, (H // scale, W // scale), order=1, anti_aliasing=True)
    return lr.astype(np.float32)


def configurator_superres(f):
    out = {}
    ...
    # Normalize HR image parameters to [-1, 1]
    ...
    out["parameters"] = p.reshape((B, H * W))  # HR flattened
    out["summary_conditions"] = degraded[..., None]  # (B, 14, 14, 1)

    return out


def inpainting_mask(image, mask_size=14):
...
def build_trainer(checkpoint_path, args, forward_train=None):
    ...
    trainer = Trainer(amortized_posterior, configurator=configurator_superres, checkpoint_path=checkpoint_path)
