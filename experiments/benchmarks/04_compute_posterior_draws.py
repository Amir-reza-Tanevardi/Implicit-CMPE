...
eval_estimators = list(
    set(estimators + ['cmpe10', 'cmpe30', 'cmpe_implicit10', 'cmpe_implicit30']).difference({'cmpe'})
)
    ...
    for task in tasks:
        for budget in simulation_budgets:
            for estimator in estimators:
                print(f"we are here: {task} {estimator} {budget} run {run_idx}")
            ...
            for budget in simulation_budgets:
                for estimator in estimators:
                    ...
                    if estimator == 'cmpe':
                        ...
                        eval_dict[task][budget]['cmpe_implicit10'] = {}
                        eval_dict[task][budget]['cmpe_implicit30'] = {}
                        eval_dict[task][budget]['cmpe_implicit10']['posterior_samples'] = trainer.amortizer.sample_implicit(eval_data, n_steps=10, n_samples=num_posterior_samples, to_numpy=False)
                        eval_dict[task][budget]['cmpe_implicit30']['posterior_samples'] = trainer.amortizer.sample_implicit(eval_data, n_steps=30, n_samples=num_posterior_samples, to_numpy=False)
