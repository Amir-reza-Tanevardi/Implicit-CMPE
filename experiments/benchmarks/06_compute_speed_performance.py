...
if __name__ == '__main__':
    ...
    def sample_implicit_timed(trainer, num_runs=3, **kwargs):
        t_min = np.inf

        for _ in range(num_runs):
            tic = time.time()
            samples = trainer.amortizer.sample_implicit(**kwargs)
            toc = time.time()
            t = toc - tic
            if t < t_min:
                t_min = t
            ...
            for estimator in estimators:
                ...
                if estimator == 'cmpe':
                    for step in sampling_steps:
                        ...
                        eval_dict[task][f'cmpe_implicit{step}'] = {}
                        samples, t = sample_implicit_timed(trainer, num_runs=num_runs_timed, input_dict=eval_data, n_steps=step, n_samples=num_posterior_samples, to_numpy=False)
                        eval_dict[task][f'cmpe_implicit{step}']['posterior_samples'] = samples if type(samples) == np.ndarray else samples.numpy()
                        eval_dict[task][f'cmpe_implicit{step}']['time'] = t
