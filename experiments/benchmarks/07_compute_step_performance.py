                ...
                for estimator in estimators:
                    ...
                    if estimator == "cmpe":
                        for n_steps in tqdm(sampling_steps, leave=False):
                            ...
                            eval_dict[task][budget][f"cmpe_implicit{n_steps}"] = {}
                            eval_dict[task][budget][f"cmpe_implicit{n_steps}"]["posterior_samples"] = trainer.amortizer.sample_implicit(
                                eval_data, n_steps=n_steps, n_samples=num_posterior_samples, to_numpy=False
                            )
                ...
                for estimator in estimators:
                    if estimator == "cmpe":
                        for n_steps in tqdm(sampling_steps, leave=False):
                            ...
                            c2st_vals_implicit = []
                            for i in tqdm(range(n_test_instances)):
                                c2st_vals_implicit.append(
                                    c2st(
                                        reference_posteriors[task][i],
                                        eval_dict[task][budget][f"cmpe_implicit{n_steps}"]["posterior_samples"][i],
                                        seed=0,
                                    )
                                )
                            eval_dict[task][budget][f"cmpe_implicit{n_steps}"]["c2st"] = np.array(c2st_vals_implicit)
